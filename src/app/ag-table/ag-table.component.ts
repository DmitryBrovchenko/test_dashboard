...
export class AgTableComponent implements OnInit {
  ...
  columnTypes = {
    "numeric": {valueFormatter: function(params) {return (params.value!=null) ? params.value.toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,'): params.value;
    }},
    'percent': {valueFormatter: function(params) {return (params.value!=null) ? (params.value*100).toFixed(2)+'%' : null}}
  };
